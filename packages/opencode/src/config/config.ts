  ...
  Effect.gen(function* () {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(yield* authWellKnown.all().pipe(Effect.orDie))) {
          ...
          const remote = yield* Effect.gen(function* () {
            ...
            const text = yield* substitution
              .substitute({
                text: JSON.stringify(wellknown.remote_config),
                type: "virtual",
                dir: url,
                source: wellknownURL,
                env: authEnv,
            ...
            if (!isRecord(parsed) || typeof parsed.url !== "string") return undefined
            return {
              url: parsed.url,
...
export const node = LayerNode.make(layer, [FSUtil.node, AuthWellKnown.node, Substitution.node, Account.node, Env.node, Npm.node, httpClient])
